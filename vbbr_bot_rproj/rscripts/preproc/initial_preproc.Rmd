...
df_ipChecked <- df_file %>%
select(prolific_id, IPAddress) %>%
mutate(ip_info = map(IPAddress, ~check_ip(.x, api_key))) %>%
unnest(ip_info)
```
...
df_participants_file <- read_csv(filepath_participants) %>% 
  filter(prolific_id %in% df_file$prolific_id)

filepath_messages <- here("data/raw/sql_export/messages.csv")
df_messages_file <- read_csv(filepath_messages) %>% 
  filter(participant_id %in% df_participants_file$id) %>% 
  left_join(df_participants_file %>% 
...
df_messages_file %>% 
  filter(prolific_id=="66e3420030eb97c9e561a351") %>% 
  view
